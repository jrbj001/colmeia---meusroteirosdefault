...
console.log('Criando views do sistema de usuários...\n');

(async () => {
  try {
    const pool = await getPool();
    
    // View 1: usuario_completo_vw
    console.log('1/4 Criando usuario_completo_vw...');
    ...
    `);
    console.log('✅ usuario_completo_vw criada');
    
    // View 2: usuario_permissoes_vw
    console.log('2/4 Criando usuario_permissoes_vw...');
    await pool.request().query(`
    ...
    `);
    console.log('✅ usuario_permissoes_vw criada');
    
    // View 3: area_sistema_hierarquia_vw
    console.log('3/4 Criando area_sistema_hierarquia_vw...');
    await pool.request().query(`
    ...
    await pool.request().query(`
      ...
      LEFT JOIN [serv_product_be180].[area_sistema_dm] ap ON a.area_pai_pk = ap.area_pk;
    `);
    console.log('✅ area_sistema_hierarquia_vw criada');
    
    // View 4: perfil_permissoes_resumo_vw
    console.log('4/4 Criando perfil_permissoes_resumo_vw...');
    await pool.request().query(`
    ...
    `);
    console.log('✅ perfil_permissoes_resumo_vw criada');
    
    console.log('\n✅ TODAS AS VIEWS CRIADAS COM SUCESSO!\n');
    
    // Testar as views
    console.log('Testando as views...\n');
    
    const usuarios = await pool.request().query(`SELECT TOP 3 * FROM [serv_product_be180].[usuario_completo_vw]`);
    console.log('📊 Amostra de usuario_completo_vw:');
    console.table(usuarios.recordset);
    
    const permissoesResumo = await pool.request().query(`SELECT * FROM [serv_product_be180].[perfil_permissoes_resumo_vw]`);
    console.log('\n📊 Resumo de perfis e permissões:');
    console.table(permissoesResumo.recordset);
