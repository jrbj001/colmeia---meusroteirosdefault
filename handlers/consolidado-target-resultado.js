...
async function consolidadoTargetResultado(req, res) {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        FROM [serv_product_be180].[reportDataIndicadoresConsolidadoTarget_dm_vw]
    ...
    const summary = await pool.request()
      ...
      .query(`
        ...
        FROM [serv_product_be180].[reportDataIndicadoresConsolidadoTargetSummary_dm_vw]
        WHERE report_pk = @report_pk
      `);
    ...
    console.error('Erro ao consultar consolidado VP+Indoor no target:', error);
    res.status(500).json({ success: false, message: 'Erro ao consultar consolidado no target', error: error.message });
  }
}

module.exports = consolidadoTargetResultado;
